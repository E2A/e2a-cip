...
export const dataIO = {
  data () {
    return {
      entityTypes: ['activities', 'setup', 'globalrecommendations', 'assessments', 'comments']
    }
  },
  computed: {
    emptyCsvRow: function () {
      ...
      this.entityTypes.forEach(entity => {
        const data = this.$store.getters[`entities/${entity}/query`]().model.fields()
  ...
  methods: {
    runExportData: function (exportType = 'json') {
      ...
      if (exportType === 'json') {
        const defaultFilename = this.$t('fileUpload.jsonFileName', { timestamp: timestamp })
        ...
        const defaultFilename = this.$t('fileUpload.csvFileName', { timestamp: timestamp })
        let csvString = ''
    ...
    exportFile: function (data, name, type) {
      if (type !== 'csv' && type !== 'json') throw new Error('exportFile: type argument must be "csv" or "json"')
    ...
    parseFileData: function (fileData) {
      ...
      if (fileData.fileObject.type === 'text/csv' || extension === 'csv') {
        ...
        for (const entity of this.entityTypes) {
          ...
          if (entityData) {
            this.$store.dispatch(`entities/${entity}/create`, { data: entityData })
          }
        }
    ...
    parseCsvData: function (csvData) {
      ...
      manyEntities.forEach(elem => {
        ...
        const entityData = entity.substring(entity.indexOf('\n') + 1, entity.length)
