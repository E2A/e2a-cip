  ...
  methods: {
    getItemCount: function(objectType) {
      if (objectType === "all") {
        ...
        const setupCount = this.$store.getters[
          "entities/setup/query"
        ]().count();
        const recommendationCount = this.$store.getters[
          "entities/recommendations/query"
        ]().count();
        const assessmentsCount = this.$store.getters[
          "entities/assessments/query"
        ]().count();
        return (
          activityCount + setupCount + recommendationCount + assessmentsCount
        );
      } else {
    ...
    getItemValue: function(objectType, objectField, objectId) {
      ...
      if (objectId) {
        objectItem = this.$store.getters[`entities/${objectType}/query`]().find(
          objectId
        );
      } else {
        objectItem = this.$store.getters[
          `entities/${objectType}/query`
        ]().first();
      }
    ...
    setupPresent: function() {
      ...
      if (setup) {
        return (
          setup.title && setup.countryCode && setup.role && setup.currencyCode
        );
      } else {
        return false;
      }
    ...
    getCountryIndicator: function(indicatorId) {
      ...
      const countryIndicator = this.$store.getters[
        "entities/countryindicators/query"
      ]()
        .where("indicatorId", indicatorId)
    ...
    getChartData: function(activityTypes) {
      ...
      const totalActivities = this.$store.getters[
        "entities/activities/query"
      ]().count();
      const totalBudget = this.getBudgetTotal(
        this.$store.getters["entities/activities/all"]()
      ); // can be 0

      // Get counts, budget and percents for each activityType
      for (const activityType of activityTypes) {
        const activityCount = this.$store.getters["entities/activities/query"]()
          .where("type", activityType.key)
          .where("youthCentric", true)
          .count();
        const activityTypesObjects = this.$store.getters[
          "entities/activities/query"
        ]()
          .where("type", activityType.key)
          .where("youthCentric", true)
        ...
        activityTypeData.push({
          ...
          count: activityCount,
          countPercent: totalYouthActivities
            ? (activityCount / totalYouthActivities).toFixed(3)
            : 0,
          budgetAmount: this.getBudgetTotal(activityTypesObjects),
          budgetPercent: totalYouthBudget
            ? (
      ...
      youthCentricBudgetData.push({
        ...
        youthCentricPercent: totalBudget
          ? (youthCentricBudget / totalBudget).toFixed(3)
          : 0.5,
      ...
      const youthCentricCount = this.$store.getters[
        "entities/activities/query"
      ]()
        .where("youthCentric", true)
        .count();
